...
func (p *Parser) Search(ctx context.Context, rule models.Rule, opts parser.SearchOptions) ([]models.Offer, error) {
	...
	sa, err := p.handshake(ctx, endpoint, rule.Query)
	if err != nil {
		return nil, fmt.Errorf("handshake: %w", err)
	}
	var offers []models.Offer
	for page := 1; len(offers) < opts.Limit; page++ {
		raw, nextSA, err := p.fetchPage(ctx, endpoint, rule.Query, page, sa)
		if err != nil {
			return nil, err
		}
		sa = nextSA
...
func (p *Parser) handshake(ctx context.Context, endpoint, query string) (string, error) {
	resp, err := p.get(ctx, endpoint, query, 1, "")
	if err != nil {
		return "", err
	}
	if resp.Data.SA == "" {
...
func (p *Parser) fetchPage(ctx context.Context, endpoint, query string, page int, sa string) ([]apiItem, string, error) {
	resp, err := p.get(ctx, endpoint, query, page, sa)
	if err != nil {
		return nil, "", err
	}
...
func (p *Parser) get(ctx context.Context, endpoint, query string, page int, sa string) (*apiResponse, error) {
	params := url.Values{}
	...
	req, err := p.client.Request(ctx, "GET", endpoint+"?"+params.Encode())
	if err != nil {
		return nil, err
	}
